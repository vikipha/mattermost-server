...
// GetPluginsEnvironment returns the plugin environment if plugins are enabled and initialized.
//
// Manually acquire the plugins lock to access the plugins environment even if plugins are
// disabled.
func (a *App) GetPluginsEnvironment() *plugin.Environment {
	...
	a.pluginsLock.RLock()
	defer a.pluginsLock.RUnlock()

	return a.pluginsEnvironment
}

func (a *App) SetPluginsEnvironment(pluginsEnvironment *plugin.Environment) {
	a.pluginsLock.Lock()
	defer a.pluginsLock.Unlock()

	a.pluginsEnvironment = pluginsEnvironment
}

func (a *App) SyncPluginsActiveState() {
	a.pluginsLock.RLock()
	pluginsEnvironment := a.pluginsEnvironment
	a.pluginsLock.RUnlock()
