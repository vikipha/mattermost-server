...
func testLogJoinEvent(t *testing.T, ss store.Store) {
	...
	user = *store.Must(t, ss.User().Save(&user)).(*model.User)
...
func testGetUsersInChannelAtChannelMemberHistory(t *testing.T, ss store.Store) {
	...
	store.Must(t, ss.ChannelMemberHistory().LogJoinEvent(user.Id, channel.Id, joinTime))
...
func testPermanentDeleteBatch(t *testing.T, ss store.Store) {
	...
	store.Must(t, ss.ChannelMemberHistory().LogJoinEvent(user2.Id, channel.Id, joinTime))
	...
	rowsDeleted := store.Must(t, ss.ChannelMemberHistory().PermanentDeleteBatch(leaveTime, math.MaxInt64)).(int64)
